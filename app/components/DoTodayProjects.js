...
const DoTodayProjects = () => {
  ...
  const getTodayProjects = (projects) => {
    return projects.filter((project) => {
      const isProjectStalled = project.projectContextId && project.projectDue && isDateToday(project.projectDue);
  ...
  return (
    <Box>
      <List spacing={3}>
        {getTodayProjects(coreDoProjects).length === 0 ? (
          <ListItem>No stalled projects available</ListItem>
        ) : (
          getTodayProjects(coreDoProjects).map((project) => {

...
const DecideUndecidedProjects = () => {
  ...
  return (
    <Box>
      <List spacing={3}>
        {getUndecidedProjects(coreProjects).length === 0 ? (
          <ListItem>No undecided projects available</ListItem>
        ) : (
          getUndecidedProjects(coreProjects).map((project) => {
                ...
                <ListItem display="flex" alignItems="center">
                  ...
                  <HStack spacing={2} ml={2}>
                    ...
                    {taskCounts.decidedTasks > 0 && (
                      <Box position="relative">
                        <IconButton
                          icon={<FaTasks color="orange" />}
                          aria-label="Decided Tasks"
                          onClick={() => setExpandedTaskProjectId(expandedTaskProjectId === project.projectId ? null : project.projectId)}
                        />
                        <Badge position="absolute" top="-1" right="-1" colorScheme="orange">{taskCounts.decidedTasks}</Badge>
                      </Box>
                    )}
                    <IconButton
                      isDisabled
